package cn.klmb.demo.framework.sms.core.client.impl;

import cn.klmb.demo.framework.sms.core.client.SmsClient;
import cn.klmb.demo.framework.sms.core.client.SmsClientFactory;
import cn.klmb.demo.framework.sms.core.client.impl.aliyun.AliyunSmsClient;
...
public class SmsClientFactoryImpl implements SmsClientFactory {
     ...
     * <p>
     * 注意，一些场景下，需要获得某个渠道类型的客户端，所以需要使用它。 例如说，解析短信接收结果，是相对通用的，不需要使用某个渠道编号的 {@link #channelIdClients}
     */
    ...
    public AbstractSmsClient createSmsClient(SmsChannelProperties properties) {
        SmsChannelEnum channelEnum = SmsChannelEnum.getByCode(properties.getCode());
        Assert.notNull(channelEnum, String.format("渠道类型(%s) 为空", channelEnum));
        // 创建客户端
        switch (channelEnum) {
            case ALIYUN:
                AliyunSmsClient aliyunSmsClient = new AliyunSmsClient(properties);
                aliyunSmsClient.init();
                return aliyunSmsClient;
        }
        // 创建失败，错误日志 + 抛出异常
        log.error("[createSmsClient][配置({}) 找不到合适的客户端实现]", properties);
        throw new IllegalArgumentException(
                String.format("配置(%s) 找不到合适的客户端实现", properties));
    }

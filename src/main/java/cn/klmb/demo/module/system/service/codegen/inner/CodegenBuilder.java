...
/**
 * 代码生成器的 Builder，负责： 1. 将数据库的表 {@link TableInfo} 定义，构建成 {@link CodegenTableDO} 2. 将数据库的列
 * {@link TableField} 构定义，建成 {@link CodegenColumnDO}
 */
@Component
public class CodegenBuilder {
    ...
    /**
     * {@link cn.klmb.demo.framework.base.core.entity.KlmbBaseDO} 的字段
     */
     ...
     * 列表操作的条件，不需要传递的字段
     */
    private static final Set<String> LIST_OPERATION_EXCLUDE_COLUMN = Sets.newHashSet("id", "bizId");
    /**
     * 列表操作的结果，不需要返回的字段
     */
    ...
    static {
        Arrays.stream(ReflectUtil.getFields(KlmbBaseDO.class))
    ...
    private void processColumnOperation(CodegenColumnDO column) {
        ...
        COLUMN_LIST_OPERATION_CONDITION_MAPPINGS.entrySet().stream()
                ...
                .findFirst().ifPresent(
                        entry -> column.setListOperationCondition(entry.getValue().getCondition()));
        if (column.getListOperationCondition() == null) {
            column.setListOperationCondition(CodegenColumnListConditionEnum.EQ.getCondition());
        }
        // 处理 listOperationResult 字段，默认增加bizId返回
        boolean resultColumn = !LIST_OPERATION_RESULT_EXCLUDE_COLUMN.contains(column.getJavaField())
                || isPrimaryKey;
        column.setListOperationResult(resultColumn);
    }
    ...
    private void processColumnExample(CodegenColumnDO column) {
        // id
        if (StrUtil.endWithAnyIgnoreCase(column.getJavaField(), "bizId")) {
            column.setExample(IdUtil.fastSimpleUUID());
            return;
        }
        // price、count 等可能是整数的后缀
        if (StrUtil.endWithAnyIgnoreCase(column.getJavaField(), "id")) {
        ...
        if (StrUtil.endWithIgnoreCase(column.getColumnName(), "url")) {
            column.setExample("https://xxx.xxx.xxx");
            return;
        }
        // reason
        ...
        if (StrUtil.endWithAnyIgnoreCase(column.getColumnName(), "description", "memo", "remark")) {
            column.setExample(randomEle(new String[]{"朝辞白帝彩云间，千里江陵一日还",
                    "飞流直下三千尺，疑是银河落九天"}));
            return;
        }
    }
